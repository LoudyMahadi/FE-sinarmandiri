    ...
    <div>
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          />
          <button className="bg-white border border-gray-300 rounded-md px-4 py-2 text-sm">
            Export Excel
          </button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <p className="text-xs text-gray-400 mb-1">Total Jenis Barang</p>
            <p className="text-2xl font-semibold text-gray-800">{totalItem}</p>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <p className="text-xs text-gray-400 mb-1">Total Stok</p>
            <p className="text-2xl font-semibold text-gray-800">{totalStok}</p>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <p className="text-xs text-gray-400 mb-1">Stok Menipis</p>
            <p className="text-2xl font-semibold text-red-500">{stokMenipis}</p>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <p className="text-xs text-gray-400 mb-1">Lokasi Toko</p>
            ...
            <tbody>
              {filtered.map((row, i) => (
                <tr key={row.id} className="border-t border-gray-100">
                  ...
                  <td className="px-4 py-3 text-gray-400">{row.product?.sku}</td>
                  <td className="px-4 py-3">{row.store?.name}</td>
                  <td className="px-4 py-3">Rp{row.product?.price?.toLocaleString('id-ID')}</td>
                  <td className="px-4 py-3">
                    <span className={row.quantity <= row.min_threshold ? 'text-red-500 font-medium' : ''}>
                      {row.quantity}
                    </span>
                  </td>
                </tr>
