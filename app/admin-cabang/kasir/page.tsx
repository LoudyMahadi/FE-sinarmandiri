...
export default function KasirCabangPage() {
  ...
  const [showPinModal, setShowPinModal] = useState(true); // wajib login PIN dulu pas buka halaman
  const [pinInput, setPinInput] = useState('');
  const [pinError, setPinError] = useState('');
  const [pinLoading, setPinLoading] = useState(false);

  // ambil store_id milik admin cabang yang login, dan daftar produk
  useEffect(() => {
  ...
  const handleSwitchStaff = () => {
    ...
    setCart([]); // reset keranjang biar gak ketuker transaksi antar staf
  };
  ...
  return (
    <div>
      ...
      {/* konten kasir, cuma bisa diakses kalau sudah ada activeStaff */}
      {activeStaff && (
          ...
          <div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              ...
              <button
                ...
                className="w-full mt-4 bg-blue-600 text-white text-sm font-medium py-2.5 rounded-md hover:bg-blue-700"
              >
      ...
      {/* modal PIN — muncul kalau belum ada kasir aktif */}
      {showPinModal && (
